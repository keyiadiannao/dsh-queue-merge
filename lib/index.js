...
function apply(ctx, config) {
	...
	ctx.on("agent/pre-step", async ({ agent, messages, signal }, next) => {
		...
		if ((policies.get(sessionId)?.mode ?? config.defaultMode) !== "merge") return next();
		const inbox = agent.inbox;
		const pending = inbox?.nextTurn ?? [];
		if (messages.length + pending.length < Math.max(1, config.minQueueForMerge)) return next();
		const downstream = await next();
		if (downstream === null || typeof downstream !== "object" || downstream.kind !== "enter") return downstream;
		let extra = [];
		if (inbox?.splice && pending.length > 0) try {
			extra = inbox.splice("next-turn", 0, pending.length, []);
		} catch {}
		const brief = await synthesizeBrief(ctx, agent, [...messages, ...extra], config, signal);
